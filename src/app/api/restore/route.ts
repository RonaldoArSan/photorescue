  ...
  try {
    ...
    if (!process.env.REPLICATE_API_TOKEN) {
      return NextResponse.json({ 
        ...
        details: 'Configure a variável REPLICATE_API_TOKEN no arquivo .env' 
      }, { status: 500 });
    }
    ...
    console.log('✅ Restauração com Replicate concluída!');

    // O output do GFPGAN é uma URL direta para a imagem restaurada
    const restoredUrl = typeof output === 'string' ? output : (output as any)?.output || (output as any);
